...
export function sos_mode(conf, parserConf) {
    ...
    function tokenStringFactory(delimiter) {
		...
		// this does not work for type script. Need to fix.
    ...
    var external = {
        ...
        indent: function (state, textAfter) {
            if (state.tokenize !== tokenBase)
                return state.tokenize.isString ? 1 : 0;
            var scope = top(state), closing = scope.type === textAfter.charAt(0);
            if (scope.align !== null)
			    // the 1 was previously CodeMirror.Pass, do not know what is going on...
...
export default function define_sos_mode(CodeMirror, MIME_TYPE) {
	let sos_keywords = commonKeywords.concat(commonBuiltins);
	CodeMirror.registerHelper("hintWords", "sos", sos_keywords);
	CodeMirror.defineMode("sos", sos_mode);
	CodeMirror.defineMIME(MIME_TYPE, "sos");
