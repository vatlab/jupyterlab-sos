...
const sosKeywords = ['input', 'output', 'depends', 'parameter'];
const sosActionWords = [
  'script',
  'download',
  'run',
  'bash',
...
const sosFunctionWords = ['sos_run', 'logger', 'get_output'];
...
const sosDirectives = sosKeywords.map(x => x + ':');
const sosActions = sosActionWords.map(x => new RegExp('^\\s*' + x + ':'));
const sosMagics = sosMagicWords.map(x => '%' + x);

function findMode(mode: string): any {
  const modeMap = Manager.manager.get_config('sos.kernel_codemirror_mode');
  if (modeMap) {
    if (mode in modeMap) {
      return modeMap[mode];
...
function findModeFromFilename(filename: string): any {
  ...
  let mode;
  if ((m = /.+\.([^.]+)$/.exec(val))) {
    const info = (CodeMirror as any).findModeByExtension(m[1]);
    if (info) {
      mode = info.mode;
    }
  } else if (/\//.test(val)) {
    const info = (CodeMirror as any).findModeByMIME(val);
    if (info) {
      mode = info.mode;
    }
  } else {
    ...
    token: function (stream: any, state: any) {
      if (state.in_python) {
        ...
        while (stream.next() && !stream.match(state.sigil.left, false)) {
          // Continue until we find the left sigil
        }
        return null;
      }
    }
...
export function sos_mode(
  conf: CodeMirror.EditorConfiguration,
  parserConf: any
) {
  const sosPythonConf: any = {};
  for (const prop in parserConf) {
    if (Object.prototype.hasOwnProperty.call(parserConf, prop)) {
  ...
  if ('base_mode' in parserConf && parserConf.base_mode) {
    const spec = findMode(parserConf.base_mode);
    if (spec) {
      let modename = spec;
      ...
      if (!Object.prototype.hasOwnProperty.call(CodeMirror.modes, modename)) {
        ...
        (CodeMirror as any).requireMode(modename, () => {}, {});
      }
  ...
  if (base_mode) {
    const python_mode = (CodeMirror as any).getMode(
      {},
      {
        name: 'python',
        version: 3
    ...
    const overlay_mode = markExpr(python_mode);
    return {
      startState: function () {
        return {
          sos_mode: true,
      ...
      token: function (stream: any, state: any) {
        if (state.sos_mode) {
          if (stream.sol()) {
            ...
            for (let i = 0; i < sosMagics.length; i++) {
              if (stream.match(sosMagics[i])) {
                if (sosMagics[i] === '%expand') {
                  // %expand, %expand --in R
                  if (
                    stream.eol() ||
                    stream.match(/\s*(-i\s*\S+|--in\s*\S+)?$/, false)
                  ) {
                    ...
                    const found = stream.match(
                      /\s+(\S+)\s+(\S+)\s*(-i\s*\S+|--in\s*\S+)?$/,
                      false
                    );
                    if (found) {
                      state.overlay_state.sigil = {
                        left: found[1].match(/^.*[A-Za-z]$/)
                          ? found[1] + ' '
                          : found[1],
                        right: found[2].match(/^[A-Za-z].*$/)
                          ? ' ' + found[2]
                          : found[2]
                      };
                    } else {
    ...
    return {
      ...
      copyState: function (state: any) {
        return {
          ...
          inner_state:
            state.inner_mode &&
            (CodeMirror as any).copyState(state.inner_mode, state.inner_state),
          // for overlay
          basePos: state.basePos,
          baseCur: null,
          overlayPos: state.overlayPos,
      ...
      token: function (stream: any, state: any) {
        if (stream.sol()) {
          ...
          if (sl === '[') {
            ...
            const mode = findMode(state.sos_state.slice(9).toLowerCase());
            if (mode) {
          ...
          if (
            ...
            ((state.inner_mode && stream.indentation() < state.sos_indent) ||
              state.sos_state === 'unknown_language')
          ) {
            state.inner_mode = null;
            state.sos_state = null;
          }
          for (let i = 0; i < sosDirectives.length; i++) {
          ...
          for (let i = 0; i < sosActions.length; i++) {
            if (stream.match(sosActions[i])) {
              // switch to submode?
              if (stream.eol()) {
                // really
                const mode = findMode(stream.current().slice(0, -1));
                if (mode) {
          ...
          if (stream.peek() === ']') {
            ...
            if (stream.eol()) {
              ...
              stream.backUp(1);
              const it = base_mode.token(stream, state.base_state);
          ...
          const mode_string = state.sos_state.slice(6).toLowerCase();
          ...
          if (
            ...
            state.sos_state =
              'start ' + findModeFromFilename(found ? found[1] : found);
          }
          const token = base_mode.token(stream, state.base_state);
          // if it is end of line, ending the starting switch mode
          if (stream.eol() && stream.peek() !== ',') {
            // really
            const mode = findMode(state.sos_state.slice(6).toLowerCase());
            if (mode) {
        ...
        if (state.sos_state === 'unknown_language') {
          ...
          const it = 'sos_script ';
          if (!state.overlay_state.sigil) {
            const st = state.inner_mode.token(stream, state.inner_state);
